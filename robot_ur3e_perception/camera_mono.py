...
class ShowingImage(Node):
    ...
    def __init__(self):
        ...
        self.image_sub = self.create_subscription(Image, "/wrist_rgbd_depth_sensor/depth/image_raw", self.camera_callback, 10)

    def camera_callback(self, msg):
        try:
            cv_image = self.bridge_object.imgmsg_to_cv2(msg, desired_encoding="passthrough")
            cv2.imshow('image from camera',cv_image)
            #modifying the image
            gray_image = cv2.normalize(cv_image, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
